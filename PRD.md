...
- **Range as a range, not a fixed number:** a car's real-world range varies by trim/battery pack/model variant (e.g. a Tesla Model Y might span 300–600 km depending on configuration), so the data model should hold a min–max range per car (showing all variants) rather than one fixed figure, and the budget/range recommendation UI should move from a single desired-range input to a range slider. This touches the `Car` model, the browse/filter screen, and likely the comparison and detail screens too — candidate for a dedicated future iteration, not a small add-on.
- **Price as a function of features, not a fixed number:** similarly, a car's price varies depending on which features/trim are selected — a base price plus feature-driven variation — rather than one static figure per car. This is related to (but distinct from) the existing comparison-mode feature toggling, and should be considered together with the range-as-a-range change above since both affect how a car's identity is modeled and displayed across browse, comparison, and detail.
